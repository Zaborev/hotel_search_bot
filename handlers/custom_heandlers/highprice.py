...
def lowprice(message: Message) -> None:
    ...
    bot.send_message(message.from_user.id,
                     f'{message.from_user.full_name}, введите город для поиска лучших отелей')
...
def get_photo_count(message: Message) -> None:
    if int(message.text) in range(1, 6):
        ...
        bot.send_message(message.from_user.id, 'Sorry, могу показать только от 1 до 5 фотографий отеля :(')
